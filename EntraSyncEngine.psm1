...
function Initialize-EntraFramework {
    ...
    try {
        Import-Module ADSync -UseWindowsPowerShell -ErrorAction SilentlyContinue
...
function Write-EntraHeader {
    ...
    if ($Global:EntraState.StagingMode -eq $true) {
        Write-Host -NoNewline " ENTRA SYNC ENGINE | $Title | $Timestamp" -ForegroundColor White
        Write-Host " | " -NoNewline -ForegroundColor White
        Write-Host "*** STAGING MODE ACTIVE ***" -ForegroundColor White -BackgroundColor DarkBlue
    }
    else {
        Write-Host " ENTRA SYNC ENGINE | $Title | $Timestamp" -ForegroundColor White
    }
...
function Invoke-SyncAnalyzer {
    ...
    try {
        ...
        if (-not $Data[0].PSObject.Properties.Match('OMODT').Count -or -not $Data[0].PSObject.Properties.Match('DN').Count) {
            Write-EntraLog "[-] The CSV does not appear to be generated by CSExportAnalyzer (Missing 'OMODT' or 'DN' columns)." "Red"
            Pause; return
        }
